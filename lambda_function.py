...
def get_recommendation(risk_level):
    """
    Function recommends the level of investment in stocks and bonds based on risk appetite
    """
    if(risk_level == 'None'):
        return f"100% bonds (AGG), 0% equities (SPY)"
    elif(risk_level == 'Low'):
        return f"60% bonds (AGG), 40% equities (SPY)"
    elif(risk_level == 'Medium'):
        return f"40% bonds (AGG), 60% equities (SPY)"
    else:
        return f"20% bonds (AGG), 80% equities (SPY)"
...
def recommend_portfolio(intent_request):
    ...
    # YOUR CODE GOES HERE!
    # if the invokation Lambda function is recommendPortfolio then execute the logic here
    if source == 'recommendPortfolio':
        slots = get_slots(intent_request) # get slots' values
        ...
        return close(
            ...
            "Fulfilled",
            {
                ...
                for your choice of risk level {}: {}
                """.format(
                    risk_level, dollars, get_recommendation(risk_level)
                ),
            },
        )
